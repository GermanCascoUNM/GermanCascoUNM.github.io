...
/*:
 * @author S_Rank_Crazy
 * @plugindesc v0.3.1 Displays indicators above events
...
(function($, $core)
{
	...
	$core.registerPlugin($, "SRCrazy_EventIndicators", "0.3.1", "2016-01-19", true, ["SRCrazy_Core"]);
	...
	{
		var key = event._mapId + "::" + event.eventId();
	...
	{
		var eventKey = eventMapId + "::" + eventId;
			...
			if (args && args.length > 0)
			{
				...
				switch (cmd)
				{
					case "enable":
						...
						if (eventId)
						{
							if ($._activeIndicators[eventId])
							{
								$._activeIndicators[eventId].setVisible(show.toLowerCase());
							}
						}
						// All indicators
						else
					...
					case "toggle":
						...
						if (eventId)
						{
							if ($._activeIndicators[eventId])
							{
								$._activeIndicators[eventId].toggleVisible();
							}
						}
						// All indicators
						else
...
(function($, $core)
{
	...
	{
		this.setVisible(!this.this.visible);
	};
